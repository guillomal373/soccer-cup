...
function mostrarEquipo(equipo) {
  ...
  // Colores
  const colors = equipo.colors || ['#165b3a', '#21a36c', '#ffffff'];
  // Estadísticas del equipo (calculadas desde los partidos)
  fetch('data/matches.json')
    .then(response => response.json())
    .then(matchesData => {
      let stats = { played: 0, won: 0, draw: 0, lost: 0, goalsFor: 0, goalsAgainst: 0, points: 0 };
      matchesData.matches.forEach(match => {
        let isA = match.teamA === equipo.id;
        let isB = match.teamB === equipo.id;
        if (isA || isB) {
          let gf = isA ? match.scoreA : match.scoreB;
          let ga = isA ? match.scoreB : match.scoreA;
          if (typeof gf === 'number' && typeof ga === 'number') {
            ...
            else { stats.draw++; stats.points += 1; }
          }
        }
      });
      container.innerHTML = `
        <div class="team-page">
          <div class="team-header">
            <img src="${equipo.logo}" alt="${equipo.name} logo" class="team-logo-lg">
            <h1>${equipo.name}</h1>
          ...
          <div class="team-stats">
            ...
            <ul class="team-stats-list">
              <li>PJ: ${stats.played}</li>
              <li>PG: ${stats.won}</li>
              <li>PE: ${stats.draw}</li>
              <li>PP: ${stats.lost}</li>
              <li>GF: ${stats.goalsFor}</li>
              <li>GC: ${stats.goalsAgainst}</li>
              <li>PTS: ${stats.points}</li>
            </ul>
          </div>
          <h2>Jugadores</h2>
          <ul class="team-players-list">
            ${equipo.players.map(j => `<li>${j.name} <span class='pos'>${j.position}</span></li>`).join('')}
          </ul>
        </div>
      `;
    });
